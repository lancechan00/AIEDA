"""把解析后的 KiCad 项目转换为 image-text 检索样本（Stage 2）。"""
...
from ..transformers import BoardTransformer


class ImageTextPairBuilder:
    """构建 image-text retrieval 数据，与 graph-text 同结构、同 split，便于对比。"""

    def __init__(self, seed: int = 7, negatives_per_sample: int = 2, image_size: int | tuple = 512) -> None:
        self.seed = seed
    ...
    def build_pairs(self, parsed_data_dir: str, output_dir: str) -> Dict[str, Any]:
        ...
        if len(project_files) < 3:
            raise ValueError("image-text 数据至少需要 3 个解析后的项目，用于 board-level 划分")
        ...
        for split_name, files in split_projects.items():
            ...
            self._render_board_images(projects, images_dir)
            pool = self._build_negative_text_pool(projects)
            samples = self._build_split_samples(projects, images_dir, output_path, pool, random_gen)
    ...
    def _render_board_images(self, projects: List[Dict[str, Any]], images_dir: Path) -> None:
        for project in projects:
            board_id = project["project_name"]
            board_data = project.get("board", {})
            out_path = images_dir / f"{board_id}.png"
            if out_path.exists():
                continue
            try:
                img = self.board_transformer.board_to_image(board_data, image_size=self.image_size)
                img.save(out_path, "PNG")
            except Exception:
                pass  # 空板或异常时跳过，后续样本会缺失 image_path
    ...
    def _build_split_samples(
        ...
        for project in projects:
            board_id = project["project_name"]
            image_path = images_dir / f"{board_id}.png"
            rel_image_path = str(image_path.relative_to(output_path)) if image_path.exists() else None
            ...
            for text in positives:
                ...
                if rel_image_path:
                    sample["image_path"] = rel_image_path
                sample_id += 1
                if rel_image_path:
